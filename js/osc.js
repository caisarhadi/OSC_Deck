...
export function throttleOSC() {
    ...
    if (now - lastSendTime > 33) { // ~30fps 
        const msg = `ws.send: /5axis [${state.tx.toFixed(2)}, ${state.ty.toFixed(2)}, ${state.tz.toFixed(2)}, ${state.ry.toFixed(2)}, ${state.rz.toFixed(2)}] | /knobs [${state.k1.toFixed(2)}, ${state.k2.toFixed(2)}, ${state.k3.toFixed(2)}]`;
        logBuffer.push(msg);
